...
def lookup():
    ...
    if request.method == "POST":
        if form.poke_name.data:
            pokemon = form.poke_name.data.lower()
            if Pokemon.poke_name_in_db(pokemon):
            ...
            pokemon = random.randint(1, 898)
            if Pokemon.poke_id_in_db(pokemon):
                pokemon_dict = Pokemon.query.filter_by(poke_id_num=pokemon).first()
                return render_template("lookup.html.j2", pokemon_table=pokemon_dict, form=form)
...
def start_battle(id):
    ...
    random.shuffle(Battle.user_squad)
    random.shuffle(Battle.opp_squad)
